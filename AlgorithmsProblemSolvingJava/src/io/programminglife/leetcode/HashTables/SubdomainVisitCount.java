...
public class SubdomainVisitCount {
    ...
    public static List<String> subdomainVisits(String[] cpdomains) {
        List<String> subdomainsCount = new ArrayList<>();
        Map<String, Integer> dict = new HashMap<>();

        for(String cpdomain: cpdomains) {
            String[] spaceSeparated = cpdomain.split("\\s");
            String domain = spaceSeparated[1];
            int count = Integer.parseInt(spaceSeparated[0]);

            if(dict.containsKey(spaceSeparated[1])) {
                dict.put(domain, dict.get(domain) + count);
            } else {
                dict.put(domain, count);
            }

            while(domain.contains(".")) {
                int firstDotPos = domain.indexOf(".");
                int domainSize = domain.length();
                domain = domain.substring(firstDotPos + 1, domainSize);
        ...
        for(String key : dict.keySet()) {
            subdomainsCount.add(String.valueOf(dict.get(key)) + " " + key);
        }
